...
VIDEO_PATH = SCRIPT_DIR.parent / "videos" / "testvideo0.mp4"
...
MAIN_DISPLAY_MAX_DIM = 900
CROP_DISPLAY_MAX_DIM = 400
...
def olcekle(img, max_dim):
    """Görüntünün uzun kenarını max_dim'e sığdırır, ekranı komple kaplamasın diye."""
    h, w = img.shape[:2]
    olcek = max_dim / max(h, w)
    if olcek >= 1:
        return img
    return cv2.resize(img, (int(w * olcek), int(h * olcek)))
            ...
            if kisi_listesi:
                ...
                if x2 > x1 and y2 > y1:
                    ...
                    for (px1, py1, px2, py2) in telefontespiti(kirpilmis):
                        # Telefon tespitinin çıktısı kırpılmış görüntünün kendi
                        # koordinatlarında; hem kırpılmış görüntüye hem de tam kareye
                        # (offset eklenerek) çiziliyor.
                        cv2.rectangle(kirpilmis, (px1, py1), (px2, py2), (255, 0, 0), 2)

                        tx1, ty1, tx2, ty2 = px1 + x1, py1 + y1, px2 + x1, py2 + y1
                        cv2.rectangle(kare, (tx1, ty1), (tx2, ty2), (255, 0, 0), 2)

                        cx, cy = int(tx1 / 2 + tx2 / 2), int(ty1 / 2 + ty2 / 2)
                        if cv2.pointPolygonTest(bolge, (cx, cy), False) > 0:
                            konusuluyor = True
                            cv2.polylines(kare, [bolge], True, (102, 0, 153), 3)

                    cv2.imshow("kirpilmis", olcekle(kirpilmis, CROP_DISPLAY_MAX_DIM))

            if konusuluyor:
                etiket_ciz(kare, LABEL_TEXT)

            cv2.imshow("kamera", olcekle(kare, MAIN_DISPLAY_MAX_DIM))
